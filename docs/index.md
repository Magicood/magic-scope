...
库内已收录 **94** 个组件。本页收录其中 35 个的静态预览(深色奥术主题)+ 参数(props)表——**完整的 94 组件交互式演示以展示站为准**(每个组件带真实源码 demo 与参数旋钮,props 直接从源码抽取)。把鼠标移上去看**发光与过渡**,按 Tab 看**聚焦光环**。

> 打开展示站:仓库内运行 `pnpm play:showcase`——可拖参数旋钮实时改组件、逛主题画廊、一键切换主题 / 密度 / 动效 / 光影。
