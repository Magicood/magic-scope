...
- **Text 多态文字原语**:`as`(多态标签)+ `asChild`(Slot 合并到子),把可控文字属性收成 props——字族(sans/serif/mono/display)、流式字号档、字重(语义档 + 数值可变字体)、斜体、tone 着色、对齐、行高、字距、transform、下划线/删除线、单行 `truncate`(头/尾)+ 多行 `lineClamp`、折行 `wrap`(balance/pretty)、`whitespace`、`breakWord`、`hyphens`、`dir`、数字变体(`numeric` tabular 等宽)、`smallCaps`、`selectable`,以及**魔法文字**:`gradient`(tone 渐变 / aurora 流光)、`glow`(辉光)、`stroke`(描边)——全复用 tone/fx/动效档,带降级。
